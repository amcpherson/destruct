...
void EstCatalog::ReadEsts(istream& in)
{
	...
	while (getline(in, line))
	{
		...
		Segment alignment;
		alignment.start = SAFEPARSE(int, estFields[16]) + 1;
		alignment.end = SAFEPARSE(int, estFields[17]);
...
void EstCatalog::FilterContainedInEstIslands(istream& in, ostream& out)
{
	...
	while (getline(in, line))
	{
		...
		searchSegment.start = SAFEPARSE(int, alignmentFields[15]) + 1;
		searchSegment.end = SAFEPARSE(int, alignmentFields[16]);
