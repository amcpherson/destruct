...
MatePairEM::MatePairEM(int minClusterSize, double precision)
{
	mMinClusterSize = minClusterSize;
	mMinProbFactor = normalpdf(-r8_normal_01_cdf_inverse((1 - precision) / 2), 0.0, 1.0);	
	mKMeansIter = 1000;
	mLambda = 0.1;
	mTolerance = 0.001;
	mKMax = 10;
...
double MatePairEM::LogLikelihood() const
{
	...
	for (int i = 0; i < mN; i++)
	{
		for (int j = 0; j < mK; j++)
		{
			exponents[j][i] = -0.5 * pow((mA[j] + mB[j] - mX[i] - mY[i] - mU[i]) / mS[i], 2.0) - mLambda * max(0.0, mX[i] - mA[j]) - mLambda * max(0.0, mY[i] - mB[j]);
		}
	}
...
bool MatePairEM::MaxLikelihood(const vector<double>& R, const vector<double>& RXO, const vector<double>& RYO, double& a, double& b) const
{
	...
	while (i < (int)SX.size() && j < (int)SY.size())
	{
		if (i + 1 < (int)SX.size() && mXO[i] == mXO[i+1])
		{
			i++;
			continue;
		}
		
		if (j + 1 < (int)SY.size() && mYO[j] == mYO[j+1])
		{
			j++;
			continue;
		}
		...
		if (SX[i] == SY[j])
		{
			...
			if (i + 1 < (int)SX.size() && j + 1 < (int)SY.size())
			{
	...
	{
		partial[i] = RXYU - NKS * (CX[i] + CY[i]) + mLambda * CS[i];
	}
	
	int minindex = 0;
	while (minindex < (int)partial.size())
	{
	...
	double aplusb = (RXYU + mLambda * CS[minindex]) / NKS;
...
void MatePairEM::DoClustering(const MatePairVec& matePairs, IntegerTable& clusters)
{
	...
	for (int j = 0; j < mK; j++)
	{
		...
		for (int i = 0; i < mN; i++)
		{
			double prob = PairProbability(mX[i], mY[i], mU[i], mS[i], mA[j], mB[j]);
			
			if (prob > mMinProbFactor / mS[i])
			{
				cluster.push_back(i);
			}
		...
		else if (mMinClusterSize == 1 && bestRep > -1)
		{
			clusters.push_back(vector<int>());
			clusters.back().push_back(bestRep);
		}
	}
}
