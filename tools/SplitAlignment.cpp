...
bool SplitAlignment::Initialize(const LocationVec& alignPair, const Sequences& reference,
								...
								double fragmentLengthStdDev, int minReadLength, int maxReadLength)
{
	...
	for (int clusterEnd = 0; clusterEnd <= 1; clusterEnd++)
	{
		...
		for (set<string>::const_iterator transcriptIter = transcripts.begin(); transcriptIter != transcripts.end(); transcriptIter++)
		{
...
void SplitAlignment::ReadCandidateRefSeqs(istream& in, SplitAlignmentMap& splitAlignments)
{
	...
	while (getline(in, line))
	{
		...
		int alignStrand = SAFEPARSE(int, fields[3]);
		int alignSeqStart = SAFEPARSE(int, fields[4]);
		int alignSeqLength = SAFEPARSE(int, fields[5]);
		int seqStrand = SAFEPARSE(int, fields[6]);
...
void SplitAlignment::ReadCandidateMateRegions(istream& in, SplitAlignmentMap& splitAlignments)
{
	...
	while (getline(in, line))
	{
		...
		int strand = SAFEPARSE(int, fields[3]);
		int start = SAFEPARSE(int, fields[4]);
		int end = SAFEPARSE(int, fields[5]);
...
void SplitAlignment::ReadAlignments(istream& in, SplitAlignmentMap& splitAlignments)
{
	...
	while (getline(in, line))
	{
		...
		int id = SAFEPARSE(int, fields[0]);
		int readID = SAFEPARSE(int, fields[1]);
		int refSplitFirst = SAFEPARSE(int, fields[2]);
		int refSplitSecond = SAFEPARSE(int, fields[3]);
		int readSplitFirst = SAFEPARSE(int, fields[4]);
		int readSplitSecond = SAFEPARSE(int, fields[5]);
		int score = SAFEPARSE(int, fields[6]);
