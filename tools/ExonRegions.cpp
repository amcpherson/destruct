...
void ExonRegions::Read(const string& exonRegionsFilename)
{
	ifstream exonRegionsFile(exonRegionsFilename.c_str());
	if (!exonRegionsFile.good())
	{
	...
	while (getline(exonRegionsFile, line))
	{
		lineNumber++;
		...
		{
			Region exon;
			exon.start = SAFEPARSE(int, exonRegionsFields[fieldIndex - 1]);
			exon.end = SAFEPARSE(int, exonRegionsFields[fieldIndex]);
...
void ExonRegions::GetRegionGenes(const string& chromosome, const Region& region, set<string>& genes) const
{
	...
	for (int bin = startBin; bin <= endBin; bin++)
	{
		...
		for (StringVecConstIter transcriptIter = lookupIter->second.begin(); transcriptIter != lookupIter->second.end(); transcriptIter++)
		{
			...
			{
				genes.insert(mTranscriptGene.find(*transcriptIter)->second);
			}
		}
	}
}

void ExonRegions::GetRegionTranscripts(const string& chromosome, const Region& region, set<string>& transcripts) const
{
	if (mTranscriptLookup.find(chromosome) == mTranscriptLookup.end())
	{
		return;
	...
	for (int bin = startBin; bin <= endBin; bin++)
	{
		...
		for (StringVecConstIter transcriptIter = lookupIter->second.begin(); transcriptIter != lookupIter->second.end(); transcriptIter++)
		{
			if (Overlap(mTranscriptRegion.find(*transcriptIter)->second,region))
			{
				transcripts.insert(*transcriptIter);
			}
		}
	}
}
